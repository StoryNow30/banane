...
function runNatif(file,label){
  const Segments=require(B+'tools/merge-segments.cjs'),Lab=require(B+'tools/placement-lab.cjs');
  const O=require(B+'src/continuity-observer.js'),Shadow=require(B+'src/gcv1-shadow.js'),L=require(B+'src/lot-decision.js'),C=require(B+'vendor/capture-core.js');
  ...
  for(const record of records){
    ...
    const d=L.decideCut({capture,science:Shadow.scientificProposeBoth(capture),anchors,Shadow});
    if(d.anchor)anchors.push({identity:id,positions:d.positions});
    if(!d.positions)continue;
    const row={source:label,kind:'natif',cut:id.cut,...describe(d),judged:false};
...
function runLot(dir,label,relecture){
  const A=require(B+'tools/acceptance-report.cjs'),L=require(B+'src/lot-decision.js');
  const seen=new Map();
  const deps={L:{...L,decideCut:args=>{const d=L.decideCut(args);seen.set(args.capture.identity.cut,d);return d;}}};
  ...
  for(const x of r.lots[0].rows){if(x.excluded||!x.lot?.wouldApply)continue;const d=seen.get(x.cut);if(!d)continue;
    const judged=!!x.lot.errors;rows.push({source:label,kind:'pilote',cut:x.cut,...describe(d),judged,
...
function run(argv=process.argv.slice(2)){
  const out=argv[0],rows=[];if(!out||out.startsWith('--'))throw Error('Usage : SORTIE.json --natif F=libellé [...] --lot DOSSIER=libellé[@RELECTURE] [...]');
  for(let i=1;i<argv.length;i++){const t=Date.now();
    if(argv[i]==='--natif'){const [f,l]=argv[++i].split('=');rows.push(...runNatif(f,l||f));console.log('natif',l,Math.round((Date.now()-t)/1000)+' s');}
    else if(argv[i]==='--lot'){const [f,rest]=argv[++i].split('='),[l,rel]=(rest||f).split('@');rows.push(...runLot(f,l,rel));console.log('lot',l,Math.round((Date.now()-t)/1000)+' s');}
    else throw Error('Argument inconnu : '+argv[i]);}
  const groups={natif:rows.filter(x=>x.kind==='natif'),pilote:rows.filter(x=>x.kind==='pilote')};
  const result={format:'banane-choice-anchor-study-v1',wrongMm:WRONG_MM,
...
module.exports={runNatif,runLot,tally,rules,RULES,run};
