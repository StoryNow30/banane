...
function study(lot,{gap=3,stables=false,guard=false,tolerance=8,window=5}={}){
  ...
  const withValidated=A.report([lot],{replay:true,preferReplay:true,replayDeps:deps});
...
function run(argv=process.argv.slice(2)){
  let lotArg=null,relecture=null,gap=3,stables=false,guard=false,tolerance=8,window=5,json=null;
  for(let i=0;i<argv.length;i++){if(argv[i]==='--lot')lotArg=argv[++i];else if(argv[i]==='--gap')gap=Number(argv[++i]);
    else if(argv[i]==='--relecture')relecture=argv[++i];else if(argv[i]==='--stables')stables=true;else if(argv[i]==='--garde')guard=true;else if(argv[i]==='--tolerance')tolerance=Number(argv[++i]);else if(argv[i]==='--fenetre')window=Number(argv[++i]);else if(argv[i]==='--json')json=argv[++i];else throw Error('Argument inconnu : '+argv[i]);}
  if(!lotArg)throw Error('Usage : --lot DOSSIER[=libellé] [--relecture DOSSIER] [--gap 3] [--stables] [--garde [--tolerance 8] [--fenetre 5]] [--json SORTIE]');
  const [dir,label]=lotArg.split('='),r=study(A.loadLot(dir,label||dir,relecture),{gap,stables,guard,tolerance,window});
