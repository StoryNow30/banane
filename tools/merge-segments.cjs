...
const loaded = files.map(f => {
  ...
  return { file: f, doc, seg: doc.segment || {} };
});
loaded.sort((a, b) =>
  String(a.seg.stamp || '').localeCompare(String(b.seg.stamp || '')) ||
...
for (const { file, doc, seg } of loaded) {
  ...
  for (const id of (full.cloudIds || full.session?.cloudIds || [])) declared.add(id);

  // État de session le plus avancé : celui qui connaît le plus de nuages.
  const score = (full.session?.cloudIds || []).length;
  if (score >= baseScore) {
    baseScore = score;
