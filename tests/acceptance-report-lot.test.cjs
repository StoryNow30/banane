...
test('règles du rejeu selon la version du lot ; --regles-actuelles les impose',()=>{
  ...
  const old={gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false},now={gaugeGuardMm:20,minTop:5,anchorRule:'decided',crossing:false,framed:false};
  ...
  assert.deepEqual(A.rulesFor('4.7.11',true),{pairGuard:true,chainMm:15,...now,anchorRule:'placed',crossing:true,framed:true});assert.deepEqual(A.rulesFor('4.7.15'),{pairGuard:true,chainMm:15,...old});
  ...
  // 4.7.19 (KI-058) : lecteur « passage à niveau » en dernier recours et voie encadrée ; consignés par la v6.
  assert.deepEqual(A.rulesFor('4.7.19'),{pairGuard:true,chainMm:15,...now,anchorRule:'placed',crossing:true,framed:true});
  const v6={version:'lot-decision-v6',pairGuard:true,chainMm:15,gaugeGuardMm:20,minTop:5,anchorRule:'placed',crossing:true,framed:false};
  assert.deepEqual([A.lotRules([{lotObservation:v6}],'4.7.19')].map(r=>[r.crossing,r.framed,r.source])[0],[true,false,'lot']);
  assert.deepEqual([A.lotRules([{lotObservation:{...v6,version:'lot-decision-v5'}}],'4.7.19')].map(r=>[r.crossing,r.framed])[0],[false,false],'un lot v5 n\'avait ni lecteur ni voie encadrée');
});
...
test('règles du rejeu : consignées par le lot d\'abord, version de l\'export en dernier recours',()=>{
  ...
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v2',pairGuard:false}),'4.7.14'),{pairGuard:false,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v2'}),'4.7.11'),{pairGuard:true,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v1'}),'4.7.12'),{pairGuard:true,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'export'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v1'}),'4.7.11'),{pairGuard:false,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'export'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v2',pairGuard:false}),'4.7.14',true),{pairGuard:true,chainMm:15,gaugeGuardMm:20,minTop:5,anchorRule:'placed',crossing:true,framed:true,source:'actuelles'});
});
...
test('règles du rejeu : chainMm consigné, 10 mm pour tout lot antérieur à la 4.7.15',()=>{
  const obs=lo=>[{lotObservation:lo}];
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v3',pairGuard:true,chainMm:15}),'4.7.15'),{pairGuard:true,chainMm:15,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v3',pairGuard:true,chainMm:12}),'4.7.15'),{pairGuard:true,chainMm:12,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v3'}),'4.7.15'),{pairGuard:true,chainMm:15,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v1'}),'4.7.15'),{pairGuard:true,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'export'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v2',pairGuard:true}),'4.7.15'),{pairGuard:true,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules([],'4.7.15'),{pairGuard:true,chainMm:15,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'export'});
  assert.deepEqual(A.lotRules([],'4.7.6'),{pairGuard:false,chainMm:10,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'export'});
});
...
test('règles du rejeu : garde d\'écartement voisin et minTop consignés, absents avant la 4.7.16',()=>{
  const obs=lo=>[{lotObservation:lo}];
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v4',pairGuard:true,chainMm:15,gaugeGuardMm:20,minTop:5}),'4.7.16'),{pairGuard:true,chainMm:15,gaugeGuardMm:20,minTop:5,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v4',pairGuard:true,chainMm:15,gaugeGuardMm:null,minTop:15}),'4.7.16'),{pairGuard:true,chainMm:15,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules(obs({version:'lot-decision-v3',pairGuard:true,chainMm:15}),'4.7.16'),{pairGuard:true,chainMm:15,gaugeGuardMm:null,minTop:15,anchorRule:'decided',crossing:false,framed:false,source:'lot'});
  assert.deepEqual(A.lotRules([],'4.7.16'),{pairGuard:true,chainMm:15,gaugeGuardMm:20,minTop:5,anchorRule:'decided',crossing:false,framed:false,source:'export'});
});
