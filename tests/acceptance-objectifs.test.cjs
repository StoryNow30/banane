...
test('§14 G : C5, le bilan des curseurs, est rapporté avec C1 à C4',()=>{
  ...
  for(const part of [...Object.values(r.parts),r.total]){
    ...
    assert.ok(part.c5.reviews.includes('audit/curseurs-lot-2026-09-24.md'));assert.deepEqual(part.c5.withoutVariationReview,['frameGap','frameAnchors','validatedMinInliers']);}
});
