...
function bridge({launcherStatus}={}){let listener,onMessage,pill,counter=0,clock=0,actualVisible=true;const timers=new Map(),sent=[],traces=[];
 ...
 function command(action,respond){const replies=[];onMessage({kind:'page-command',action,args:[]},{id:'test'},respond?r=>respond(r,replies):r=>replies.push(r));return {request:sent.at(-1),replies,timer:[...timers.keys()].at(-1)};}
